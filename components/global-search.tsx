...
async function searchEntities(query: string): Promise<SearchResult[]> {
    ...
    try {
        // Search users
        const [usersRes, businessRes, customerRes, programRes] = await Promise.all([
            http.get('/user', { params: { search: query, limit: 5 } }),
            http.get('/business', { params: { search: query, limit: 5 } }),
            http.get('/customer', { params: { search: query, limit: 5 } }),
            http.get('/loyalty-program', { params: { search: query, limit: 5 } }),
        ]);
...
export function GlobalSearch() {
    ...
    // Debounce the query feeding the network request so we fire once typing
    // settles, instead of a (4-endpoint) search on every keystroke.
